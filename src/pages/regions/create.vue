<template>
  <div class="q-pa-md row items-start q-gutter-md" v-if="CheckRole('AreaCreate')">

    <div class="text-h6 fit">Bölge Oluştur! 
      <q-btn
    ...
    regionSave () {
      if (this.model !== null) {
        ...
        if (veri[0].data.length < 3) {
          ...
          this.$axios.post('regions/create', { region_detail })
            .then((result) => {
              this.$q.loading.hide()
              this.$q.dialog({

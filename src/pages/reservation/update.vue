<template>
  <div class="q-pa-md" v-if="CheckRole('ReservationUpdate')">
    <q-stepper
      v-model="step"
      vertical
      color="primary"
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        rez:this.$route.params.reservation,
    ...
    methods: {
      reservationSave() {
        ...
        this.$axios.post('reservation/update', { NewReservation })
          .then((result) => {
            let storeIndex=this.$store.state.reservationList.findIndex(item => (item.id==this.rez.id))
            this.$store.state.reservationList[storeIndex].transfer_status = 7;
            this.$q.loading.hide()
            this.$q.dialog({
      ...
      step3FN () {
        ...
        this.passengerChange()
        this.step = this.step + 1
      },
      step4FN(){
        if(this.passengerControl()){
          this.passengerTitleUpdate()
          let times=this.rez.departure_date.split(" ")[1].split(":")
          this.departure = this.rez.departure_date.split(" ")[0]
          this.departure_time = times[0] + ":" + times[1]
          this.reservation_note = this.rez.reservation_note
          if(this.transfer_direction_model.id==2 || this.transfer_direction_model.id == 4) {
            this.room_number_departure = this.rez.flight_or_room
            // console.log("Oda")
          }else{
            // console.log("Uçuş" , this.transfer_direction_model.id)
            this.flight_number_departure = this.rez.flight_or_room
          }

          if(this.rez.departure_paying==1){
            this.payment_person_departure = "hotel"
          }else{
            this.payment_person_departure = "musteri"
            this.payment_costumer_departure = this.rez.departure_pay_price
            this.payment_currency_departure = this.rez.departure_pay_currency
          }
      ...
      passengerChange(){
        ...
        for(var i=0;i<this.people_number;i++) {
          if(this.rez.passengers.length>i){
            this.passengers.push(
              {
                country: (this.countries[this.countries.findIndex(search => search.id == this.rez.passengers[i].country)]) ? this.countries[this.countries.findIndex(search => search.id == this.rez.passengers[i].country)] : null,
                gender: (this.gender[this.countries.findIndex(search => search.id == this.rez.passengers[i].gender)]) ? this.gender[this.countries.findIndex(search => search.id == this.rez.passengers[i].gender)] : null,
                identity: (this.rez.passengers[i].identity) ? this.rez.passengers[i].identity : null,
                firstname:  (this.rez.passengers[i].firstname) ? this.rez.passengers[i].firstname : null,
                lastname:   (this.rez.passengers[i].lastname) ? this.rez.passengers[i].lastname : null,
                phone:      (this.rez.passengers[i].phone) ? this.rez.passengers[i].phone : null,
                email:      (this.rez.passengers[i].emai) ? this.rez.passengers[i].emai : null
              });
          }else{
      ...
      getCars(){
        ...
        this.$axios.get(car_url).then((response => {
          ...
          this.car_model=this.cars[this.cars.findIndex(search => (search.car_id == this.rez.car_id))]
      ...
      priceCalc(){
        ...
        this.$axios.post('prices/data', { price_detail })
          .then((response => {
            ...
            this.people_number=this.people[this.rez.passenger_size]
            return true
          })).catch((error) => {
      ...
      getAreas () {
        ...
        this.$axios.get(url).then((response => {
          ...
          if(this.transfer_direction_model.id==parseInt(this.rez.direction)){
            this.nereden_model=this.nereden[this.nereden.findIndex(search => (search.area_id == this.rez.departure_area_id))]
            this.nereye_model=this.nereye[this.nereye.findIndex(search => (search.area_id == this.rez.arrival_area_id))]
            this.ChangeArea()
            this.hotel_model=this.ShowHotels[this.ShowHotels.findIndex(search => (search.hotel_id == this.rez.hotel_id))]
          }else{
      ...
      setDataModifier(){
        this.transfer_direction_model=this.transfer_direction[this.transfer_direction.findIndex(search => (search.id == this.rez.direction))]
        this.transfer_type_model=this.transfer_type[0]
      }
    },
    mounted () {
      if(!this.$route.params.hasOwnProperty('reservation')){
        this.$router.push({"name" : "reservationList"})
      }
      this.setDataModifier()
      // this.getCars()
